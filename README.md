...
- **事件增强** — 备注、时间范围（开始/结束时间）、全天事件
- **重复事件** — 每天/每周/每月/每年重复，可设结束日期；实例化存储，支持「仅此事件 / 整个系列」编辑与删除（每月重复自动处理月末，如 1/31 → 2/28）
- **拖拽调整** — 月/周视图拖拽事件改日期；周视图拖拽到时间槽可同时改时间（自动保持原时长时间）
- **日视图时间刻度轴** — 事件按开始时间落在 24 小时刻度轴上，方块高度对应时间范围；全天或未设置时间的事件置顶显示；月/周视图的事件条仅显示标题（不显示时间）
- **搜索 / 筛选** — 按标题或备注关键字、日期范围、颜色、完成状态组合筛选，结果可直接跳转日期或勾选完成
...
- **统一流程** — 周/月/季度/年度四种周期使用同一流程：将该时间段内的**全部事件**提交给 AI，由 AI 自行整合并合并**相同或相似**的事件（如同一项目的多次记录、标题相近的条目），不按天罗列、不漏项
- **手动选择时间** — 四种周期均支持手动选择起止日期（默认按当前周期填充）
- **事件预览** — 生成前只读展示将提交给 AI 的全部事件，并支持补充说明（未完成原因、下期计划、重点事项等），AI 会结合生成
- **数据聚合** — 统计完成率、颜色分类统计、全部事件清单
- **AI 总结** — 接入 OpenAI 兼容协议（DeepSeek / 通义 / OpenAI / 本地 Ollama），输出「周期总结 / 本期做的事（合并后）/ 下期要做的事 / 建议」；长度预算随事件数量自适应
- **自定义提示词** — 弹窗内展示默认提示词模板（含 `{周期}`、`{事项清单}` 等占位符），可自由修改：本次生成立即生效，或「保存为我的默认」持久化到个人账号
- **防截断** — 检测到输出被生成长度截断时自动以更大额度重试一次，仍不足则明确提示，不再静默返回不完整内容
- **自动降级** — 未配置 AI 或调用失败时，自动降级为按标题合并的统计型报告，功能不失效
...
└── frontend-react/              # React 前端项目
    ...
    └── src/
        ...
        │   ├── MonthView.tsx   # 月视图
        │   ├── WeekView.tsx    # 周视图
        │   ├── DayView.tsx     # 日视图（24 小时刻度轴，事件按时间定位，全天事件置顶）
        │   ├── EventModal.tsx  # 事件新增/编辑弹窗（含色盘自定义颜色、备注/时间范围/全天/重复）
        │   ├── EventDetailModal.tsx # 事件详情弹窗
        │   ├── SearchModal.tsx # 搜索/筛选弹窗（关键字 + 日期范围 + 颜色 + 完成状态）
        │   ├── ImportModal.tsx # 导入弹窗（含模板下载）
        │   ├── ExportModal.tsx # 导出弹窗（月份选择/导出全部）
        │   └── ReportModal.tsx # AI 总结弹窗（手动选时间/全部事件预览/Markdown 渲染/复制/下载）
        └── pages/              # 页面组件
            ├── Login.tsx       # 登录页
